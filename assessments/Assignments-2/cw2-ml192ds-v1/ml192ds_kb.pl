...
%% facts for skill
fact( [python,  subclass_of, skill] ).
fact( [prolog,  subclass_of, skill] ).
fact( [matlab,  subclass_of, skill] ).
fact( [math,    subclass_of, skill] ).
fact( [logic,   sunclass_of, skill] ).
fact( [coding,  subclass_of, skill] ).
fact( [writing, subclass_of, skill] ).

%%skills([math,logic,coding,writing,python,prolog,matlab]).
...
%% List: study hours on module of every student weekly.
...
%%rule(learning, [[X, has_module, M],[M,  requires, S], skills(S_L), member(S, S_L)]  ==> [X, learning, S]).

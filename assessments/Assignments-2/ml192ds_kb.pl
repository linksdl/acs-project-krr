...
fact( [japan,   subclass_of, country] ).
fact( [korea,   subclass_of, country] ).
fact( [us,      subclass_of, country] ).
fact( [canada,  subclass_of, country] ).
fact( [uk,      subclass_of, country] ).
fact( [france,  subclass_of, country] ).
fact( [italy,   subclass_of, country] ).
fact( [country, subclass_of, world] ).


%% Advanced computer science (AI, DataScience, Cloud Computing)
fact( [krr, subclass_of, module] ).
fact( [ml,  subclass_of, module] ).
fact( [asd, subclass_of, module] ).
fact( [pds, subclass_of, module] ).
fact( [acs, subclass_of, module] ).
...
module_study_time_list([
                   ['alice','krr',8], ['alice','ml',16], ['alice','pds',12],['alice','asd',8],
...
%rule(performance, [[X, is, student], [X, has_module, M], [M, requires, Y], [X, good_at, Y]]
%     ==> [X, pass, M] ).

%rule(performance, [[X, is, student], [X, pay_time, T, in, M], [X, has_module, M], test(T=<12)]
%     ==> [X, fial, M]).
